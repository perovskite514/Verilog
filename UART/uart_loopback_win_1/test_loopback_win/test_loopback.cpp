...
int main()
{
	...
	if (argc != 4) {
		std::wcerr << L"usage " << argv[0] << L" [com #] [baud] [file]";
		exit(1);
	}
	...
	if (WaitForSingleObject(ols_w.hEvent, INFINITE) != WAIT_OBJECT_0) {
		std::cerr << "wait for send operation failed" << std::endl;
		exit(1);
	};
	wStatus = GetOverlappedResult(hComm, &ols_w, &slen, TRUE);
	std::cout << "Write status = " << wStatus << std::endl;
	rStatus = WaitForSingleObject(ols_r.hEvent, 30000);
	if (rStatus == WAIT_OBJECT_0) {
		rStatus = GetOverlappedResult(hComm, &ols_r, &rlen, TRUE);
		std::cout << "Read status = " << rStatus << std::endl;
	} else if (rStatus == WAIT_TIMEOUT) {
		CancelIoEx(hComm, &ols_r);
		rStatus = GetOverlappedResult(hComm, &ols_r, &rlen, TRUE);
		std::cerr << "wait for recv operation time out" << std::endl;
	} else {
		std::cerr << "wait for recv operation failed" << std::endl;
		exit(1);
	}
